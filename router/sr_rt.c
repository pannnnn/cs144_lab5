...
struct sr_rt* get_next_hop(struct sr_instance *sr, uint32_t ip) {
    ...
    unsigned int longest_prefix_match = 0xffffffff;
    while (rt_walker) {
        uint32_t rt_dest = (rt_walker->dest).s_addr;
        uint32_t rt_mask = (rt_walker->mask).s_addr;
        int prefix_match;
        /* Check if rt_desk is in the routing table */
        if (((rt_dest ^ ip) & rt_mask) == 0) {
            
            prefix_match = rt_dest ^ ip;
            /* 
                Try to find the smallest distinguished number from backwards
                thus it becomes the longest_prefix_match
             */
